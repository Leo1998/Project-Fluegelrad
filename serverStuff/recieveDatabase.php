<?php
	...
	$eventsGet = $pdo->prepare("SELECT 
		...
		`sponsors`.`mail` AS host.mail` , `sponsors`.`phone` AS `host.phone` , `sponsors`.`web` AS `host.web`,
		`sponsors`.`name` AS `host.name` , `sponsors`.`description` AS `host.description` , `sponsors`.`image` AS `host.image` , `sponsors`.`id` AS `host.id` 
		FROM `events` 
