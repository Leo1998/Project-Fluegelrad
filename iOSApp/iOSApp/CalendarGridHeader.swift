...
class CalendarGridHeader: UICollectionReusableView {
	...
	*/
    private func setupMonthChanger(){
        ...
        addSubview(left)
        left.addConstraintsXY(xView: self, xSelfAttribute: .leading, xViewAttribute: .leading, xMultiplier: 1, xConstant: 8, yView: self, ySelfAttribute: .top, yViewAttribute: .top, yMultiplier: 1, yConstant: 5)

        right = UIButton()
        right.translatesAutoresizingMaskIntoConstraints = false
        right.setImage(#imageLiteral(resourceName: "ic_arrow_forward"), for: UIControlState.normal)
        addSubview(right)
        right.addConstraintsXY(xView: self, xSelfAttribute: .trailing, xViewAttribute: .trailing, xMultiplier: 1, xConstant: -8, yView: self, ySelfAttribute: .top, yViewAttribute: .top, yMultiplier: 1, yConstant: 5)

        month = UILabel()
		month.textColor = UIColor.accent()
        month.translatesAutoresizingMaskIntoConstraints = false
        addSubview(month)
        month.addConstraintsXY(xView: self, xSelfAttribute: .centerX, xViewAttribute: .centerX, xMultiplier: 1, xConstant: 0, yView: self, ySelfAttribute: .top, yViewAttribute: .top, yMultiplier: 1, yConstant: 5)
    }
	...
	*/
	private func setupWeekView(){
		...
		for week in 0...6{
			...
			tempLabel.textColor = UIColor.accent()
